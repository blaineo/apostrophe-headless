  ...
  construct: function(self, options) {
    ...
    self.addRestApiRoutes = function() {
      ...
      self.apos.app.get(endpoint, function(req, res) {
        ...
        var childDepth = self.apos.launder.string(req.query.childDepth);
        ...
        function prunePageData(pageData) {
          if( req.query.excludeFields ) {
            const toExclude = req.query.excludeFields.split(',')
            const slimData = Object.keys(pageData).reduce((object, key) => {
              if(!toExclude.includes(key)) {
                object[key] = pageData[key]
              }
              return object
            }, {})
            return slimData
        ...
        function findPage(callback) {
          return self.findForRestApi(req, childDepth).and({ slug: slug }).toObject(function(err, _page) {
            if (err) {
              return callback(err);
            }
            if (!_page) {
            ...
            const prunedPage = prunePageData(_page)
            result = prunedPage;
    ...
    self.findForRestApi = function(req, childDepth) {
      ...
      var cursor = childDepth ? self.find(req).ancestors(true).children({depth: Number(childDepth)}).published(null) : self.find(req).ancestors(true).children(true).published(null);
